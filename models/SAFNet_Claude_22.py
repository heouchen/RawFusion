"""
SAFNet_Claude_22 — 2-Pass Anchor Flow + Per-Frame Learned Merge
================================================================
Minimal intervention: only change the merge strategy, keep everything else
identical to Claude_15.

Problem with Claude_15 merge:
  avg_short = (img0_warp + img2_warp) / 2  — equal-weight average loses info
  avg_long  = (img6_warp + img8_warp) / 2  — ditto

Solution: per-frame attention merge with 5 individual weights.
Each pixel learns which of the 5 frames to trust most, instead of
averaging within exposure groups first.

  - Flow: 2-pass 3-anchor (identical to Claude_15)
  - LearnedMerge5Frame: 5 per-frame attention weights (was 3 per-exposure)
  - RefineNet 72ch: IDENTICAL to Claude_15
...
# ======================== Per-Frame Learned Merge ========================
class LearnedMerge5Frame(nn.Module):
    """Per-frame attention merge: 5 individual weights instead of 3 exposure groups.

    Input: 5 warped RGB (15ch) + mask0 (1ch) + mask8 (1ch) = 17ch
    Output: attention-weighted average of 5 frames (3ch)
    """
    def __init__(self):
        super().__init__()
        ...
        self.attn_head = nn.Conv2d(32, 5, 1, 1, 0)  # 5 per-frame weights

    def forward(self, img0_w, img2_w, img4, img6_w, img8_w, mask0, mask8):
        x = torch.cat([img0_w, img2_w, img4, img6_w, img8_w,
                        mask0, mask8], dim=1)
        feat = self.feat_net(x)
        weights = torch.softmax(self.attn_head(feat), dim=1)  # (B, 5, H, W)
...
# ======================== SAFNet_Claude_22 ========================
class SAFNet_Claude_22(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder = Encoder()
        self.decoder = DecoderDCN()
        self.refinenet = RefineNet()
        self.learned_merge = LearnedMerge5Frame()
    ...
    def forward(self, x, scale_factor=0.5, refine=True):
        ...
        # Per-frame merge (5 individual attention weights)
        img_hdr_m = self.learned_merge(
            img0_warp[:, :3], img2_warp[:, :3], img4_c[:, :3],
            img6_warp[:, :3], img8_warp[:, :3], mask0, mask8)
...
if __name__ == "__main__":
    ...
    model = SAFNet_Claude_22().to(device)
