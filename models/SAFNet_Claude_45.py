"""
SAFNet_Claude_45 — Cross-Frame Feature Attention Bridge
========================================================
Built on SAFNet_Claude_40 with enhanced fusion via cross-frame attention:
1. Add a CrossFrameAttentionBridge between stage-2 flow refinement and merge.
   Uses L1 adapted features to compute per-frame reliability weights via
   shared DW-conv + channel attention, feeding them into the merge step.
2. Slightly wider ReliableMerge3Frame (hidden_channels=40) to leverage the
   richer attention-weighted features.
...
class RepDWConvM(nn.Module):
    """Enhanced 5x5 reparameterizable DW conv."""
    def __init__(self, channels, dilation=1):
        super().__init__()
        self.channels = channels
        self.dilation = dilation
    ...
    def fuse(self):
        ...
        w_serial = torch.einsum("bcnx,bcyn->bcyx",
                                 self.conv_5w.weight.data, self.conv_5h.weight.data)
        w += w_serial
        w[:, 0, 2, 2] += 1.0
...
class RepDWConvL(nn.Module):
    ...
    def fuse(self):
        ...
        w_serial = torch.einsum("bcnx,bcyn->bcyx",
                                 self.conv_7w.weight.data, self.conv_7h.weight.data)
        w += w_serial
        w[:, 0, 3, 3] += 1.0
...
class Encoder(nn.Module):
    def __init__(self, in_channels=4):
        ...
        self.pyramid1 = nn.Sequential(
            convrelu(in_channels, 48, 3, 2, 1), RepNeXtBlock(48))
        self.pyramid2 = nn.Sequential(
            convrelu(48, 48, 3, 2, 1), RepNeXtBlock(48))
        self.pyramid3 = nn.Sequential(
            convrelu(48, 48, 3, 2, 1), RepNeXtBlock(48, expand_ratio=2.25))
...
class DecoderDCN(nn.Module):
    def __init__(self, mid_channels=96, num_blocks=2, cond=True,
        ...
        for _ in range(num_blocks):
            blocks.append(RepNeXtBlock(mid_channels, cond=cond,
                                       num_experts=num_experts, expand_ratio=expand_ratio))
...
class AnchorGroupAggregator(nn.Module):
    """Refine an exposure-group anchor frame with local same-group context."""
...
class ExposureGroupPreparer(nn.Module):
    """Compress 9 packed raw frames into 3 anchor-centric pseudo-frames."""
    def __init__(self):
        super().__init__()
...
# ======================== CrossFrameAttentionBridge ========================
class CrossFrameAttentionBridge(nn.Module):
    """Compute per-frame reliability weights from L1 encoder features.
    Takes aligned L1 features (f0w_1, f4_1, f8w_1) at 1/2 resolution,
    produces per-frame spatial reliability maps at full resolution.
    """
    def __init__(self, feat_channels=48, out_channels=1):
        super().__init__()
        in_ch = feat_channels * 3
        hidden = feat_channels
        ...
        self.ca = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(hidden, hidden // 4, 1, 1, 0, bias=True),
            nn.GELU(),
            nn.Conv2d(hidden // 4, hidden, 1, 1, 0, bias=True),
            nn.Sigmoid(),
        )
        self.head = nn.Conv2d(hidden, out_channels * 3, 1, 1, 0, bias=True)

    def forward(self, f0w, f4, f8w):
        """Returns (w0, w4, w8) each [B, 1, H, W] at feature resolution."""
        x = torch.cat([f0w, f4, f8w], dim=1)
        feat = self.compress(x)
        feat = feat * self.ca(feat)
        weights = self.head(feat)
        w0, w4, w8 = weights.chunk(3, dim=1)
        return torch.sigmoid(w0), torch.sigmoid(w4), torch.sigmoid(w8)


# ======================== Learned Merge (3-Frame) with attention bridge ========================
class ReliableMerge3FrameV2(nn.Module):
    """Enhanced v40 merge with per-frame reliability from cross-frame attention."""
    def __init__(self, feat_channels=6, hidden_channels=40, reliability_channels=1):
        super().__init__()
        # 9(imgs) + 18(feats) + 2(masks) + 2(flow_mag) + 3(exp) + 3(reliability)
        in_channels = 9 + feat_channels * 3 + 2 + 2 + 3 + reliability_channels * 3
    ...
    def forward(self, img0_w, img4, img8_w, mask0, mask8, flow0, flow8,
                feat0_w, feat4, feat8_w, rel0, rel4, rel8):
...
class RefineNet(nn.Module):
    def __init__(self, img_channels=4):
        ...
        self.conv1 = nn.Sequential(
            DeformConvRelu(img_channels + 2 + 2 + 1 + 1 + 3, c1), RepNeXtBlock(c1))
...
# ======================== SAFNet_Claude_45 ========================
class SAFNet_Claude_45(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        # NEW: cross-frame attention bridge
        self.cross_frame_attn = CrossFrameAttentionBridge(feat_channels=48, out_channels=1)
        # Wider merge with reliability inputs
        self.learned_merge = ReliableMerge3FrameV2(
            feat_channels=6, hidden_channels=40, reliability_channels=1)
    ...
    def forward_flow_mask(self, img0_c, img4_c, img8_c, scale_factor=0.5):
        ...
        # Also return L1 adapted features for cross-frame attention
        return (torch.sigmoid(up_rmask0_1), torch.sigmoid(up_rmask8_1),
                final_flow0, final_flow8,
                f0w_1, f4_1, f8w_1)
    ...
    def forward(self, x, scale_factor=0.5, refine=True):
        ...
        # Cross-frame attention bridge
        rel0, rel4, rel8 = self.cross_frame_attn(f0w_1, f4_1, f8w_1)
        # Upsample reliability maps to full resolution
        full_h, full_w = img0_c.shape[-2:]
        rel0 = F.interpolate(rel0, size=(full_h, full_w), mode='bilinear', align_corners=False)
        rel4 = F.interpolate(rel4, size=(full_h, full_w), mode='bilinear', align_corners=False)
        rel8 = F.interpolate(rel8, size=(full_h, full_w), mode='bilinear', align_corners=False)
...
def _print_profile(height=384, width=768, params_cap_m=5.0, flops_cap_g=100.0):
    ...
    model_before = SAFNet_Claude_45().to(device).eval()
