...
class ReliableMerge3Frame(nn.Module):
    ...
    def forward(self, img4_w, img0, img8_w, mask4, mask8, flow4, flow8,
                feat4_w, feat0, feat8_w):
        flow4_mag = torch.sqrt(torch.clamp(flow4.square().sum(dim=1, keepdim=True), min=1e-8)) / div_flow
        flow8_mag = torch.sqrt(torch.clamp(flow8.square().sum(dim=1, keepdim=True), min=1e-8)) / div_flow
        exp_low = mask4.new_zeros(mask4.shape)
        exp_mid = mask4.new_full(mask4.shape, -1.0)
        exp_high = mask4.new_full(mask4.shape, 1.0)
        x = torch.cat([
            img4_w, img0, img8_w,
            feat4_w, feat0, feat8_w,
            mask4, mask8,
...
class RefineNetPlus(RefineNet):
    ...
    def forward(self, img0_c, img4_c, img8_c, flow4, flow8, mask4, mask8, img_hdr_m):
        out = super().forward(img0_c, img4_c, img8_c, flow4, flow8, mask4, mask8, img_hdr_m)
        out = out + 0.10 * self.tail(out)
        return torch.clamp(out, 0, 1)

# ======================== SAFNet_Claude_40 ========================
class SAFNet_Claude_50_v2(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward_flow_mask(self, img0_c, img4_c, img8_c, scale_factor=0.5):
        ...
        up_rflow4_1 = 8.0 * resize(up_rflow4_4, scale_factor=8.0)
        up_rflow8_1 = 8.0 * resize(up_rflow8_4, scale_factor=8.0)
        up_rmask4_1 = resize(up_rmask4_4, scale_factor=8.0)
    ...
    def forward(self, x, scale_factor=0.5, refine=True):
        ...
        merge_feat4 = warp(self.merge_stem(img4_c), flow4)
        merge_feat0 = self.merge_stem(img0_c)
        merge_feat8 = warp(self.merge_stem(img8_c), flow8)

        img_hdr_m = self.learned_merge(
            img4_warp[:, :3], img0_c[:, :3], img8_warp[:, :3],
            mask4, mask8, flow4, flow8,
            merge_feat4, merge_feat0, merge_feat8)
...
def _print_profile(height=384, width=768, params_cap_m=5.0, flops_cap_g=100.0):
    ...
    model_before = SAFNet_Claude_50_v2().to(device).eval()
