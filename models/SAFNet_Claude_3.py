"""
SAFNet_Claude_3 — Multi-Frame Utilization (5 of 9 frames)
==========================================================
Key insight: current models waste 6 of 9 input frames!
Using 5 frames (0,2,4,6,8) dramatically improves SNR through temporal averaging.

Strategy:
  - Flow estimation still uses 3 anchor frames (0, 4, 8) → same encoder/decoder FLOPs
  - Extra frames (2, 6) are warped using interpolated flow (approximate alignment)
  - 5-frame HDR merge averages within each exposure group before cross-exposure fusion
  - RefineNet: standard 48ch, 5 ResBlocks (same as base SAFNet, no deformable conv)
  - Total FLOPs ≈ 78G (same as base SAFNet + negligible warp overhead)

The quality gain comes from better input signal, not more computation.
"""
import numpy as np
import torch
...
def merge_hdr(ldr_imgs, lin_imgs, mask0, mask2):
    """Standard 3-frame HDR merge (kept for compatibility)."""
...
def merge_hdr_5frame(short_ldr, mid_ldr, long_ldr,
    ...
    """
    5-frame HDR merge: average within each exposure group first (reduces noise),
    then merge across exposures using standard weighting.

    Args:
        short_ldr/lin: list of short-exposure frames (warped)
        mid_ldr/lin:   list of mid-exposure frames (reference, no warp needed for frame 4)
        long_ldr/lin:  list of long-exposure frames (warped)
    """
    # Intra-exposure temporal averaging (key SNR improvement)
...
# ======================== ResBlock (standard conv, no deform) ========================
class ResBlock(nn.Module):
    def __init__(self, channels, dilation=1, bias=True):
        super().__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(channels, channels, 3, 1, dilation, dilation, bias=bias),
            nn.PReLU(channels)
        )
        self.conv2 = nn.Conv2d(channels, channels, 3, 1, dilation, dilation, bias=bias)
...
# ======================== RefineNet (standard, same as base SAFNet) ========================
...
# ======================== SAFNet_Claude_3 ========================
class SAFNet_Claude_3(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x, scale_factor=0.5, refine=True):
        # ---- Extract 5 frames covering all 3 exposure levels ----
        img0_c = x[:, 0:4, :, :]     # Short exposure, frame 0 (reference)
        img2_c = x[:, 8:12, :, :]    # Short exposure, frame 2
        img4_c = x[:, 16:20, :, :]   # Mid exposure, frame 4
        img6_c = x[:, 24:28, :, :]   # Mid exposure, frame 6
        img8_c = x[:, 32:36, :, :]   # Long exposure, frame 8

        # ---- Flow estimation using 3 anchor frames (0, 4, 8) ----
        # Flow is from anchor to reference (frame 4 = mid ref)
        ...
        # ---- Approximate warp for intermediate frames ----
        # Frame 2 is same exposure as frame 0, spatially nearby → scale flow
        # Frame 6 is same exposure as frame 8, spatially nearby → scale flow
        ...
        if refine:
            # RefineNet still uses 3 anchor frames for feature extraction
...
if __name__ == "__main__":
    ...
    model = SAFNet_Claude_3().to(device)
