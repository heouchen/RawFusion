"""
SAFNet_Claude_39 — Alignment-Focused Claude_35 Upgrade
======================================================
Built on SAFNet_Claude_35 with the budget concentrated on the flow/mask path:
1. Stage-2 adapters become flow-aware gated adapters conditioned on normalized flow.
2. Stage-2 refinement adds a lightweight dedicated L2 decoder.
3. Low/high exposure branches use independent adapters for L2/L3/L4, while L1 stays shared.
4. Merge and RefineNet remain close to Claude_35 to isolate alignment-side gains.
...
class FlowAwareGatedAdapter(nn.Module):
    """Flow-aware adapter for stage-2 flow correction."""
    def __init__(self, channels=48):
        super().__init__()
        self.flow_norm = max(div_flow, 1.0)
        self.mix = nn.Sequential(
            nn.Conv2d(channels * 2 + 2, channels, 1, 1, 0, bias=True),
            nn.PReLU(channels),
    ...
    def forward(self, feat, feat_warp, flow):
        flow = flow / self.flow_norm
        fused = torch.cat([feat, feat_warp, flow], dim=1)
        return feat_warp + self.gate(fused) * self.mix(fused)
...
# ======================== SAFNet_Claude_39 ========================
class SAFNet_Claude_39(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.adapter_shared_l1 = FlowAwareGatedAdapter(48)
        self.adapter_low_l2 = FlowAwareGatedAdapter(48)
        self.adapter_low_l3 = FlowAwareGatedAdapter(48)
        self.adapter_low_l4 = FlowAwareGatedAdapter(48)
        self.adapter_high_l2 = FlowAwareGatedAdapter(48)
        self.adapter_high_l3 = FlowAwareGatedAdapter(48)
        self.adapter_high_l4 = FlowAwareGatedAdapter(48)
    ...
    def forward_flow_mask(self, img0_c, img4_c, img8_c, scale_factor=0.5):
        ...
        f0w_1 = self.adapter_shared_l1(f0_1, warp(f0_1, flow0_l1), flow0_l1)
        f0w_2 = self.adapter_low_l2(f0_2, warp(f0_2, flow0_l2), flow0_l2)
        f0w_3 = self.adapter_low_l3(f0_3, warp(f0_3, flow0_l3), flow0_l3)
        f0w_4 = self.adapter_low_l4(f0_4, warp(f0_4, flow0_l4), flow0_l4)
        f8w_1 = self.adapter_shared_l1(f8_1, warp(f8_1, flow8_l1), flow8_l1)
        f8w_2 = self.adapter_high_l2(f8_2, warp(f8_2, flow8_l2), flow8_l2)
        f8w_3 = self.adapter_high_l3(f8_3, warp(f8_3, flow8_l3), flow8_l3)
        f8w_4 = self.adapter_high_l4(f8_4, warp(f8_4, flow8_l4), flow8_l4)
...
def _print_profile(height=384, width=768, params_cap_m=5.0, flops_cap_g=100.0):
    ...
    model_before = SAFNet_Claude_39().to(device).eval()
