"""
SAFNet_Claude_44 — SimpleGate Refinement + Cross-Scale Skip
============================================================
Built on SAFNet_Claude_40 with two PSNR-oriented reconstruction upgrades:
1. Replace GELU with NAFNet-style SimpleGate in RefineNet residual blocks,
   doubling feature width (expand_ratio 2.5→1.25 after halving via gate)
   for richer representation without extra activation overhead.
2. Add a lightweight cross-scale skip bridge that injects the merged HDR
   embedding into the RefineNet after the first residual block.
...
# ======================== SimpleGate (NAFNet-style) ========================
class SimpleGate(nn.Module):
    """NAFNet-style gating: split channels in half and multiply."""
    def forward(self, x):
        x1, x2 = x.chunk(2, dim=1)
        return x1 * x2

# ======================== RepNeXtBlockSG (SimpleGate variant) ========================
class RepNeXtBlockSG(nn.Module):
    """RepNeXtBlock with SimpleGate instead of GELU.
    expand_ratio is doubled since SimpleGate halves the channels.
    """
    def __init__(self, channels, dilation=1, se_reduction=4, expand_ratio=2.5,
                 cond=False, num_experts=12):
        super().__init__()
        # Double expand for SimpleGate (halves output channels)
        expand_ch = int(np.ceil((channels * float(expand_ratio) * 2) / 4.0) * 4)
        effective_ch = expand_ch // 2  # After SimpleGate
        se_ch = max(effective_ch // se_reduction, 8)
        self.norm = nn.GroupNorm(1, channels)
        if cond:
            self.pw1 = CondConv2d_1x1(channels, expand_ch, num_experts)
            self.pw2 = CondConv2d_1x1(effective_ch, channels, num_experts)
        else:
            self.pw1 = nn.Conv2d(channels, expand_ch, 1, bias=True)
            self.pw2 = nn.Conv2d(effective_ch, channels, 1, bias=True)
        self.dw = ChunkConvV4(expand_ch, dilation=dilation)
        self.gate = SimpleGate()
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(effective_ch, se_ch, 1, bias=True),
            nn.GELU(),
            nn.Conv2d(se_ch, effective_ch, 1, bias=True),
            nn.Sigmoid()
        )
        self.scale = nn.Parameter(torch.ones(1, channels, 1, 1) * 0.1)
    ...
    def forward(self, x):
        ...
        y = self.pw1(y)
        y = self.dw(y)
        y = self.gate(y)  # SimpleGate instead of GELU
        y = y * self.se(y)
        y = self.pw2(y)
        return x + y * self.scale
...
# ======================== MergeSkipBridge ========================
class MergeSkipBridge(nn.Module):
    """Cross-scale skip: project merged HDR embedding to refine feature space."""
    def __init__(self, in_channels=3, out_channels=100):
        ...
        self.scale = nn.Parameter(torch.ones(1, out_channels, 1, 1) * 0.05)

    def forward(self, x):
        return self.proj(x) * self.scale

# ======================== RefineNet with SimpleGate ========================
class RefineNetSG(nn.Module):
    """RefineNet with SimpleGate blocks and cross-scale skip from merge."""
    def __init__(self, img_channels=4):
        ...
        self.merge_skip = MergeSkipBridge(3, total_c)

        self.block0 = RepNeXtBlockSG(total_c, dilation=1, cond=True, num_experts=10, expand_ratio=1.25)
        # After block0, inject merge skip
        self.blocks_rest = nn.Sequential(
            RepNeXtBlockSG(total_c, dilation=2, cond=True, num_experts=10, expand_ratio=1.25),
    ...
    def forward(self, img0_c, img4_c, img8_c, flow0, flow8, mask0, mask8, img_hdr_m):
        ...
        # Inject cross-scale skip from merge
        feat = feat + self.merge_skip(img_hdr_m)
        feat = self.blocks_rest(feat)
...
class RefineNetSGPlus(RefineNetSG):
    def __init__(self, img_channels=4):
...
# ======================== SAFNet_Claude_44 ========================
class SAFNet_Claude_44(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.refinenet = RefineNetSGPlus()
...
def _print_profile(height=384, width=768, params_cap_m=5.0, flops_cap_g=100.0):
    ...
    model_before = SAFNet_Claude_44().to(device).eval()
