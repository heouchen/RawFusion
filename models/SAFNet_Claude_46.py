"""
SAFNet_Claude_46 — ArtIR-Inspired Large-Kernel Refinement
==========================================================
Built on SAFNet_Claude_40 with enhanced receptive field in refinement:
1. Add a LargeKernelRefineBlock after the center dilation=4 RepNeXtBlock in
   RefineNet (6 blocks total instead of 5), providing a much larger ERF via
   separable 11×11 convolution on collapsed channels.
2. Double the HighResRefineTail to 2 blocks for stronger high-res cleanup.
...
# ======================== LargeKernelRefineBlock (ArtIR-inspired) ========================
class LargeKernelRefineBlock(nn.Module):
    """ArtIR-inspired block: expand → collapse to few channels → large DW kernel → expand → project.
    Uses separable 11×11 (1×11 + 11×1) for efficiency on collapsed channels.
    No SE to minimize FLOPs overhead.
    """
    def __init__(self, channels, collapse_channels=8, se_reduction=4,
                 cond=False, num_experts=10):
        super().__init__()
        self.norm = nn.GroupNorm(1, channels)
        # Pointwise collapse
        if cond:
            self.pw1 = CondConv2d_1x1(channels, collapse_channels, num_experts)
            self.pw2 = CondConv2d_1x1(channels, channels, num_experts)
        else:
            self.pw1 = nn.Conv2d(channels, collapse_channels, 1, bias=True)
            self.pw2 = nn.Conv2d(channels, channels, 1, bias=True)

        # Large-kernel separable DW on collapsed channels (very cheap)
        self.lk_h = nn.Conv2d(collapse_channels, collapse_channels, (11, 1),
                               1, (5, 0), groups=collapse_channels, bias=True)
        self.lk_w = nn.Conv2d(collapse_channels, collapse_channels, (1, 11),
                               1, (0, 5), groups=collapse_channels, bias=True)
        self.act = nn.GELU()
        # Expand back
        self.expand = nn.Conv2d(collapse_channels, channels, 1, bias=True)
        self.scale = nn.Parameter(torch.ones(1, channels, 1, 1) * 0.1)
    ...
    def forward(self, x):
        ...
        # Collapse
        y_c = self.pw1(y)
        # Large-kernel separable conv
        y_c = self.lk_h(y_c) + self.lk_w(y_c)
        y_c = self.act(y_c)
        # Expand
        y = self.expand(y_c)
        y = self.pw2(y)
...
# ======================== RefineNet with LargeKernel blocks ========================
class RefineNetLK(nn.Module):
    """RefineNet with 2 RepNeXtBlocks replaced by LargeKernelRefineBlocks."""
    def __init__(self, img_channels=4):
        ...
        self.blocks = nn.Sequential(
            ...
            LargeKernelRefineBlock(total_c, collapse_channels=4, cond=False),
            RepNeXtBlock(total_c, dilation=2, cond=True, num_experts=10, expand_ratio=2.0),
            RepNeXtBlock(total_c, dilation=1, cond=True, num_experts=10, expand_ratio=2.0),
        )
...
class RefineNetLKPlus(RefineNetLK):
    def __init__(self, img_channels=4):
...
# ======================== SAFNet_Claude_46 ========================
class SAFNet_Claude_46(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.refinenet = RefineNetLKPlus()
...
def _print_profile(height=384, width=768, params_cap_m=5.0, flops_cap_g=100.0):
    ...
    model_before = SAFNet_Claude_46().to(device).eval()
