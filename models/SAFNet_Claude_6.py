"""
SAFNet_Claude_6 — 5-Frame Temporal + NAFBlock Refinement (Best of Claude_2 & Claude_5)
=======================================================================================
Combines the two empirically best-performing variants:

From Claude_5 (better input signal):
  - 5-frame utilization (frames 0,2,4,6,8) → temporal averaging reduces noise
  - Flow interpolation for non-anchor frames (flow * 0.67)
  - merge_hdr_5frame with intra-exposure group averaging

From Claude_2 (better refinement network):
  - NAFBlock: inverted bottleneck + DW conv + SimpleGate + SCA
  - 80ch RefineNet (20+40+20) — wider feature space
  - 10 NAFBlocks — deeper refinement with multi-scale dilation
  - GroupNorm(1,C) ≈ LayerNorm for stable per-sample normalization
  - Learnable beta scaling for residual connections

Rationale:
  Claude_5's 5-frame merge produces a higher-quality coarse HDR image by
  exploiting temporal redundancy across burst frames. Claude_2's NAFBlock
  refinement then operates on this better initialization, yielding superior
  final output. The two improvements are orthogonal — one improves input
  quality, the other improves the refinement capacity.

Budget estimate: ~87G MACs, ~0.87M params (well within 100G / 5M).
Encoder/Decoder: identical to V2 (proven alignment module).
"""
import numpy as np
import torch
...
def merge_hdr_5frame(short_ldr, mid_ldr, long_ldr,
    ...
    Within each exposure group, frames are averaged first to reduce noise,
    then the standard 3-exposure weighted merge is applied across groups.
    """
...
# ======================== NAFNet Components (from Claude_2) ========================
class SimpleGate(nn.Module):
    """Split channels in half and multiply — acts as learnable gating."""
    def forward(self, x):
...
class NAFBlock(nn.Module):
    """
    NAFNet block: inverted bottleneck with depthwise conv + SimpleGate + SCA.
    Very FLOPs-efficient due to depthwise separable design.
    """
    def __init__(self, channels, dilation=1):
        super().__init__()
        dw_ch = channels * 2  # expand ratio = 2

        self.norm = nn.GroupNorm(1, channels)  # equivalent to LayerNorm
        self.pw1 = nn.Conv2d(channels, dw_ch, 1, bias=True)  # pointwise expand
        self.dw = nn.Conv2d(dw_ch, dw_ch, 3, 1, dilation, dilation,
                            groups=dw_ch, bias=True)  # depthwise
        self.sg = SimpleGate()  # dw_ch → channels
        # Simplified Channel Attention
        self.sca = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channels, channels, 1, bias=True)
        )
        self.pw2 = nn.Conv2d(channels, channels, 1, bias=True)  # pointwise project
...
# ======================== RefineNet (NAFBlock 80ch x 10, from Claude_2) ========================
class RefineNet(nn.Module):
    def __init__(self, img_channels=4):
        ...
        # 10 NAFBlocks with multi-scale dilation (from Claude_2)
        self.blocks = nn.Sequential(
            NAFBlock(total_c, dilation=1),
            NAFBlock(total_c, dilation=1),
            NAFBlock(total_c, dilation=2),
            NAFBlock(total_c, dilation=2),
            NAFBlock(total_c, dilation=4),
            NAFBlock(total_c, dilation=4),
            NAFBlock(total_c, dilation=2),
            NAFBlock(total_c, dilation=2),
            NAFBlock(total_c, dilation=1),
...
class SAFNet_Claude_6(nn.Module):
    ...
    def forward(self, x, scale_factor=0.5, refine=True):
        # ---- Extract 5 frames (from Claude_5) ----
        ...
        img2_warp = warp(img2_c, flow0 * 0.67)   # interpolated flow for frame 2
        img6_warp = warp(img6_c, flow8 * 0.67)   # interpolated flow for frame 6

        # ---- 5-frame HDR merge (from Claude_5) ----
        ...
        if refine:
            # RefineNet uses anchor frames (0, 4, 8) — same as Claude_5
