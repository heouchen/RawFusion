"""
SAFNet_Claude_31 — Dynamic Convolution (CondConv)
==============================================
Strategy: Use CondConv2d (Dynamic Convolution) to increase parameters.
Each 1x1 PW conv in RepNeXtBlock is replaced by 32 experts.
Channels are set to 48 (baseline level) to maintain FLOPs safety.

Constraints: ~4.5-5.0M params, ~98G FLOPs.
"""
import numpy as np
import torch
...
class CondConv2d_1x1(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        routing_weights = self.routing(x).view(B, self.num_experts) 
        
        combined_weight = torch.matmul(routing_weights, self.weight.view(self.num_experts, -1))
        combined_weight = combined_weight.view(B, self.out_channels, self.in_channels)
        combined_bias = torch.matmul(routing_weights, self.bias) 
        
        x = x.view(B, C, H * W)
        x = torch.bmm(combined_weight, x) + combined_bias.view(B, self.out_channels, 1)
...
class RepNeXtBlock(nn.Module):
    def __init__(self, channels, dilation=1, se_reduction=4, expand_ratio=2, cond=True, num_experts=8):
        super().__init__()
        ...
        if cond:
            self.pw1 = CondConv2d_1x1(channels, expand_ch, num_experts=num_experts)
        else:
        ...
        if cond:
            self.pw2 = CondConv2d_1x1(expand_ch, channels, num_experts=num_experts)
        else:
...
class Encoder(nn.Module):
    def __init__(self, in_channels=4):
        ...
        self.pyramid4 = nn.Sequential(
            convrelu(c, c, 3, 2, 1),
            RepNeXtBlock(c, cond=True, num_experts=8)
        )
...
class DecoderDCN(nn.Module):
    def __init__(self):
        super().__init__()
        c = 48
        self.fgdcn = FlowGuidedDCN(c)
        self.conv1 = DeformConvRelu(150, 96) # 3*48+6 = 150
...
class RefineNet(nn.Module):
    def __init__(self, img_channels=4):
        ...
        self.conv0 = nn.Sequential(convrelu(img_channels, c0), RepNeXtBlock(c0, cond=False))
        self.conv1 = nn.Sequential(
            DeformConvRelu(img_channels + 2 + 2 + 1 + 1 + 3, c1),
            RepNeXtBlock(c1, cond=False)
        )
        self.conv2 = nn.Sequential(convrelu(img_channels, c2), RepNeXtBlock(c2, cond=False))

        self.blocks = nn.Sequential(
            RepNeXtBlock(total_c, dilation=1, cond=True, num_experts=8),
            RepNeXtBlock(total_c, dilation=2, cond=True, num_experts=8),
            RepNeXtBlock(total_c, dilation=4, cond=True, num_experts=8),
            RepNeXtBlock(total_c, dilation=2, cond=True, num_experts=8),
            RepNeXtBlock(total_c, dilation=1, cond=True, num_experts=8),
        )
...
# ======================== SAFNet_Claude_31 ========================
class SAFNet_Claude_31(nn.Module):
    def __init__(self):
        super().__init__()
