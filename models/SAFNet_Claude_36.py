"""
SAFNet_Claude_36 — PFL + Routed Prompt/Adapter (UHD-processer inspired)
=========================================================================
Built on SAFNet_Claude_35 with budget-aware UHD-processer style upgrades:
1. Progressive frequency learning hooks via lightweight frequency gating.
2. Unsupervised soft routing (DALR-style) for low-rank adapter experts.
3. Dynamic frequency-space prompts (DFSP-style) via FiLM conditioning.
4. Keep large-kernel reparameterizable blocks (RepDWConvS/M/L) for deploy-time fusion.
...
class DecoderDCNLite(nn.Module):
    ...
    def __init__(self, mid_channels=72):
        ...
        self.fgdcn = FlowGuidedDCN(48)
        self.conv1 = DeformConvRelu(150, mid_channels)
        self.pre_gate = FreqPromptGate(mid_channels)
        self.block = PromptedAdapterBlock(
            mid_channels,
            cond=False,
            expand_ratio=1.75,
            num_prompts=4,
    ...
    def forward(self, f0, f1, f2, flow0, flow2, mask0, mask2):
        ...
        f2_warp = self.fgdcn(f2, flow2)
        f_in = torch.cat([f0_warp, f1, f2_warp, flow0, flow2, mask0, mask2], 1)
        f_out = self.pre_gate(self.conv1(f_in))
...
class FreqPromptGate(nn.Module):
    """Frequency-aware channel gating used by progressive frequency learning."""
...
class SoftRouter(nn.Module):
    """Unsupervised soft expert routing."""
    def __init__(self, channels, num_experts=4):
        ...
        self.router = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(hidden, num_experts, 1, 1, 0, bias=True),
        )
...
class FreqPromptBank(nn.Module):
    """Learned prompt bank for FiLM modulation."""
    def __init__(self, channels, num_prompts=4):
        super().__init__()
        self.prompt_scale = nn.Parameter(torch.zeros(num_prompts, channels))
        self.prompt_shift = nn.Parameter(torch.zeros(num_prompts, channels))
        nn.init.normal_(self.prompt_scale, std=0.02)
        nn.init.normal_(self.prompt_shift, std=0.02)

    def forward(self, routing_weights):
        gamma = torch.matmul(routing_weights, self.prompt_scale).unsqueeze(-1).unsqueeze(-1)
        beta = torch.matmul(routing_weights, self.prompt_shift).unsqueeze(-1).unsqueeze(-1)
        return gamma, beta


class RoutedLowRankAdapter(nn.Module):
    """DALR-style routed low-rank adapters for 1x1-like residual adaptation."""
    def __init__(self, channels, rank=8, num_experts=4):
        super().__init__()
        rank = max(4, int(rank))
        self.down = nn.ModuleList([
            nn.Conv2d(channels, rank, 1, 1, 0, bias=False) for _ in range(num_experts)
        ])
        self.up = nn.ModuleList([
            nn.Conv2d(rank, channels, 1, 1, 0, bias=False) for _ in range(num_experts)
        ])
        self.act = nn.GELU()
    ...
    def forward(self, x, routing_weights):
        ...
        for idx, (down, up) in enumerate(zip(self.down, self.up)):
            expert_out = up(self.act(down(x)))
            weight = routing_weights[:, idx:idx + 1].unsqueeze(-1).unsqueeze(-1)
            out = out + expert_out * weight
        return out * self.res_scale


class PromptedAdapterBlock(nn.Module):
    """DFSP + DALR hybrid wrapper around a RepNeXt block."""
    def __init__(
        self,
        channels,
        cond=False,
        ...
        super().__init__()
        self.freq_gate = FreqPromptGate(channels)
        self.router = SoftRouter(channels, num_experts=num_prompts)
        self.prompt_bank = FreqPromptBank(channels, num_prompts=num_prompts)
        self.core = RepNeXtBlock(
            channels,
            cond=cond,
            dilation=dilation,
            num_experts=num_experts,
        ...
        self.adapter = RoutedLowRankAdapter(channels, rank=adapter_rank, num_experts=num_prompts)

    def forward(self, x):
        x = self.freq_gate(x)
        routing_weights = self.router(x)
        gamma, beta = self.prompt_bank(routing_weights)
        y = x * (1.0 + gamma) + beta
        y = self.core(y)
        return y + self.adapter(y, routing_weights)
...
class RefineNet(nn.Module):
    def __init__(self, img_channels=4):
        ...
        self.pre_blocks_gate = FreqPromptGate(total_c)

        self.blocks = nn.Sequential(
            PromptedAdapterBlock(
                total_c, cond=True, dilation=1, num_experts=10, expand_ratio=2.0,
                num_prompts=4, adapter_rank=8
            ),
            PromptedAdapterBlock(
                total_c, cond=True, dilation=2, num_experts=10, expand_ratio=2.0,
                num_prompts=4, adapter_rank=8
            ),
    ...
    def forward(self, img0_c, img4_c, img8_c, flow0, flow8, mask0, mask8, img_hdr_m):
        ...
        feat = self.pre_blocks_gate(feat)
...
def _print_profile(
    ...
    model_before = SAFNet_Claude_36().to(device).eval()
