"""
SAFNet_Claude_40 — Fusion/Reconstruction-Focused Claude_35 Upgrade
==================================================================
Built on SAFNet_Claude_35 with the budget concentrated on merge and reconstruction:
1. A lightweight exposure-group preparer turns 9 packed raw frames into 3 pseudo-frames.
2. Merge becomes reliability-aware using shallow warped features, flow magnitude, masks, and exposure prior.
3. RefineNet adds a single lightweight high-resolution tail for final residual cleanup.
4. The flow/mask alignment path stays close to Claude_35 to isolate fusion-side gains.
...
class RefineNet(nn.Module):
    def __init__(self, img_channels=4):
        ...
        self.blocks = nn.Sequential(
            RepNeXtBlock(total_c, dilation=1, cond=True, num_experts=10, expand_ratio=2.5),
            RepNeXtBlock(total_c, dilation=2, cond=True, num_experts=10, expand_ratio=2.5),
            RepNeXtBlock(total_c, dilation=4, cond=True, num_experts=10, expand_ratio=2.5),
            RepNeXtBlock(total_c, dilation=2, cond=True, num_experts=10, expand_ratio=2.5),
            RepNeXtBlock(total_c, dilation=1, cond=True, num_experts=10, expand_ratio=2),
        )
...
# ======================== SAFNet_Claude_40 ========================
class SAFNet_Claude_40_v2(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward_flow_mask(self, img0_c, img4_c, img8_c, scale_factor=0.5):
        ...
        up_mask0_1 = resize(up_mask0_3, scale_factor=4.0)
        up_mask8_1 = resize(up_mask8_3, scale_factor=4.0)
...
def _print_profile(height=384, width=768, params_cap_m=5.0, flops_cap_g=100.0):
    ...
    model_before = SAFNet_Claude_40().to(device).eval()
