"""
SAFNet_Claude_37 — SAFNet_Claude_35 + MIRAGE-Style Functional Decomposition
=============================================================================
Built on SAFNet_Claude_35, with lightweight ideas adapted from MIRAGE (2505.18679):
1. Add MIRAGE-style functional decomposition block (local conv + global mix + channel MLP).
2. Keep large-kernel reparameterizable blocks (RepDWConvS/M/L) for deploy-time fusion.
3. Add training-only SPD contrastive regularization with zero inference overhead.

Target (fused): > 4M and <= 5M params, > 95G and <= 100G FLOPs
for input (1, 36, 384, 768), corresponding to output (1, 3, 768, 1536).
"""
import numpy as np
import math
import torch
...
class DecoderDCNLite(nn.Module):
    ...
    def __init__(self, mid_channels=72):
        ...
        self.block = nn.Sequential(
            RepNeXtBlock(mid_channels, cond=False, expand_ratio=1.75),
            MIRAGEStyleBlock(mid_channels, mlp_ratio=1.25, global_ratio=0.5),
        )
        self.conv_out = deconv(mid_channels, 6)
...
class FlowFeatureAdapter(nn.Module):
    """Refine warped stage-1 features for stage-2 flow correction."""
    ...
    def forward(self, feat, feat_warp):
        return feat_warp + self.mix(torch.cat([feat, feat_warp], dim=1))


class MIRAGEStyleBlock(nn.Module):
    """Functional decomposition with local/global/MLP branches."""
    def __init__(self, channels, mlp_ratio=1.25, global_ratio=0.5):
        super().__init__()
        hidden_mlp = int(np.ceil((channels * float(mlp_ratio)) / 4.0) * 4)
        hidden_glb = max(8, int(channels * float(global_ratio)))
        hidden_mix = max(8, channels // 4)
        ...
        self.global_mix = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channels, hidden_glb, 1, 1, 0, bias=True),
            nn.GELU(),
            nn.Conv2d(hidden_glb, channels, 1, 1, 0, bias=True),
            nn.Sigmoid(),
        )
        self.channel_mlp = nn.Sequential(
            nn.Conv2d(channels, hidden_mlp, 1, 1, 0, bias=True),
            nn.GELU(),
            nn.Conv2d(hidden_mlp, channels, 1, 1, 0, bias=True),
        )
        self.branch_weight = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channels, hidden_mix, 1, 1, 0, bias=True),
            nn.GELU(),
            nn.Conv2d(hidden_mix, 3, 1, 1, 0, bias=True),
        )
        self.scale = nn.Parameter(torch.ones(1, channels, 1, 1) * 0.08)

    def forward(self, x):
        y = self.norm(x)
        b_local = self.local(y)
        b_global = y * self.global_mix(y)
        b_mlp = self.channel_mlp(y)
        w = torch.softmax(self.branch_weight(y), dim=1)
        out = b_local * w[:, 0:1] + b_global * w[:, 1:2] + b_mlp * w[:, 2:3]
        return x + out * self.scale


class HighFreqStemGate(nn.Module):
    """Lightweight high-frequency gate used before high-resolution refine blocks."""
    ...
    def forward(self, x):
        ...
        high = x - low
        gate = torch.sigmoid(self.dw(high))
        return x + high * gate
...
class RefineNet(nn.Module):
    def __init__(self, img_channels=4):
        ...
        self.stem_gate = HighFreqStemGate(total_c)
...
class SAFNet_Claude_37(nn.Module):
    def __init__(self):
        ...
        self._aux_losses = {}
        self._spd_weight = 0.0
        self._spd_margin = 0.02

    def set_spd_weight(self, weight):
        self._spd_weight = float(weight)

    def _spd_contrastive_loss(self, feat_a, feat_b):
        """SPD-style contrastive regularization in covariance space."""
        bsz, ch, _, _ = feat_a.shape
        xa = feat_a.flatten(2)
        xb = feat_b.flatten(2)
        xa = xa - xa.mean(dim=-1, keepdim=True)
        xb = xb - xb.mean(dim=-1, keepdim=True)
        denom = max(xa.shape[-1] - 1, 1)
        cov_a = torch.bmm(xa, xa.transpose(1, 2)) / float(denom)
        cov_b = torch.bmm(xb, xb.transpose(1, 2)) / float(denom)

        eye = torch.eye(ch, device=feat_a.device, dtype=feat_a.dtype).unsqueeze(0)
        cov_a = cov_a + 1e-4 * eye
        cov_b = cov_b + 1e-4 * eye
        cov_a = cov_a / cov_a.diagonal(dim1=-2, dim2=-1).sum(dim=-1, keepdim=True).unsqueeze(-1)
        cov_b = cov_b / cov_b.diagonal(dim1=-2, dim2=-1).sum(dim=-1, keepdim=True).unsqueeze(-1)

        pos = F.l1_loss(cov_a, cov_b)
        if bsz > 1:
            cov_neg = cov_b.roll(shifts=1, dims=0)
        else:
            perm = torch.arange(ch - 1, -1, -1, device=feat_a.device)
            cov_neg = cov_b[:, perm][:, :, perm]
        neg = F.l1_loss(cov_a, cov_neg)
        return pos + F.relu(self._spd_margin + pos - neg)
    ...
    def forward_flow_mask(self, img0_c, img4_c, img8_c, scale_factor=0.5):
        ...
        if self.training and self._spd_weight > 0.0:
            spd_l3 = self._spd_contrastive_loss(f4_3, 0.5 * (f0w_3 + f8w_3))
            spd_l4 = self._spd_contrastive_loss(f4_4, 0.5 * (f0w_4 + f8w_4))
            self._aux_losses['spd_contrastive'] = self._spd_weight * 0.5 * (spd_l3 + spd_l4)
...
def _print_profile(
    ...
    model_before = SAFNet_Claude_37().to(device).eval()
    ...
    params_ok = (params_after > int(params_min_m * 1e6)) and (params_after <= int(params_cap_m * 1e6))
    print(
        f"Params budget ({params_min_m:.2f}M, {params_cap_m:.2f}M]: "
        f"{'PASS' if params_ok else 'FAIL'}"
    )
    ...
    flops_ok = (flops_after > float(flops_min_g) * 1e9) and (flops_after <= float(flops_cap_g) * 1e9)
    print(
        f"FLOPs budget ({flops_min_g:.2f}G, {flops_cap_g:.2f}G]: "
        f"{'PASS' if flops_ok else 'FAIL'}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--params_min_m", type=float, default=4.0, help="Param lower bound in millions.")
    parser.add_argument("--params_cap_m", type=float, default=5.0, help="Param cap in millions.")
    parser.add_argument("--flops_min_g", type=float, default=95.0, help="FLOPs lower bound in billions.")
