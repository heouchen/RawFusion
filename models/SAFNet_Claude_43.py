"""
SAFNet_Claude_43 — Claude_42 plus One Lite Prompt/Adapter Refine Block
=======================================================================
Built on SAFNet_Claude_42 as a controlled orthogonal variant:
1. Keep Claude_40 input grouping, alignment, merge, and high-resolution tail unchanged.
2. Keep the lightweight pre-refine frequency gate from Claude_42.
3. Replace only the first RefineNet residual block with a lite prompt/adapter block.
...
class FreqPromptGate(nn.Module):
    """Frequency-aware channel gating used before refine residual blocks."""
...
class FreqPromptBank(nn.Module):
    ...
    def __init__(self, channels, num_prompts=3):
        super().__init__()
...
class RoutedLowRankAdapter(nn.Module):
    """Routed low-rank adapter with fixed light rank for Claude_43."""
    def __init__(self, channels, rank=6, num_experts=3):
        super().__init__()
        rank = max(4, int(rank))
...
class PromptedAdapterBlockLite(nn.Module):
    """Single prompted refine block with fixed lightweight settings."""
    def __init__(self, channels, cond=True, dilation=1, num_experts=10, expand_ratio=2.0):
        super().__init__()
        self.freq_gate = FreqPromptGate(channels, hidden_ratio=0.5)
        self.router = SoftRouter(channels, num_experts=3)
        self.prompt_bank = FreqPromptBank(channels, num_prompts=3)
        ...
        self.adapter = RoutedLowRankAdapter(channels, rank=6, num_experts=3)
...
class ReliableMerge3Frame(nn.Module):
    def __init__(self, feat_channels=6, hidden_channels=36):
        super().__init__()
...
class RefineNet(nn.Module):
    def __init__(self, img_channels=4):
        ...
        self.pre_blocks_gate = FreqPromptGate(total_c, hidden_ratio=0.5)

        self.blocks = nn.Sequential(
            PromptedAdapterBlockLite(
...
# ======================== SAFNet_Claude_43 ========================
class SAFNet_Claude_43(nn.Module):
    def __init__(self):
        super().__init__()
...
def _print_profile(height=384, width=768, params_cap_m=5.0, flops_cap_g=100.0):
    ...
    model_before = SAFNet_Claude_43().to(device).eval()
