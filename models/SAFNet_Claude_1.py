"""
SAFNet_Claude_1 — SE Attention + Deeper RefineNet
===================================================
Key changes from SAFNet_Opt_V2:
  - Replace DeformConv in ResBlocks with standard Conv2d (saves ~13G FLOPs)
  - Add Squeeze-and-Excitation (SE) attention to each ResBlock
  - Increase ResBlock count from 5 to 6 (reinvest saved FLOPs into depth)
  - Dilation pattern: [1, 1, 2, 4, 2, 1] (wider receptive field)
  - Keep DeformConv in Decoder conv1 and RefineNet conv1 (alignment-critical)

Encoder/Decoder: identical to V2 (shared, proven alignment quality)
"""
import numpy as np
import torch
...
class SEBlock(nn.Module):
    """Squeeze-and-Excitation: global channel attention with minimal FLOPs."""
...
class ResBlockSE(nn.Module):
    """Residual block with SE attention. Standard Conv2d (no deformable)."""
    def __init__(self, channels, dilation=1, se_reduction=4):
        ...
        self.conv2 = nn.Conv2d(channels, channels, 3, 1, dilation, dilation, bias=True)
...
class RefineNet(nn.Module):
    def __init__(self, img_channels=4):
        ...
        # 6 ResBlockSE (one more than V2) with multi-scale dilation
        self.resblock1 = ResBlockSE(total_c, dilation=1)
        self.resblock2 = ResBlockSE(total_c, dilation=1)
        self.resblock3 = ResBlockSE(total_c, dilation=2)
        self.resblock4 = ResBlockSE(total_c, dilation=4)
        self.resblock5 = ResBlockSE(total_c, dilation=2)
        self.resblock6 = ResBlockSE(total_c, dilation=1)
...
# ======================== SAFNet_Claude_1 ========================
class SAFNet_Claude_1(nn.Module):
    def __init__(self):
        super().__init__()
...
if __name__ == "__main__":
    ...
    model = SAFNet_Claude_1().to(device)
