"""
SAFNet_Claude_16 — 5-Frame Direct Flow
=======================================
Based on Claude_15. Replaces 2-pass 3-anchor flow with single-pass 5-frame
direct flow estimation. All 5 frames get independent optical flow — eliminates
the 0.67x approximation for frames 2 and 6.

Pair A: (img0, img4, img8) -> flow_0->4, flow_8->4, mask0, mask8
Pair B: (img2, img4, img6) -> flow_2->4, flow_6->4, mask2, mask6
img4 encoder features shared between pairs.

Total: 5 enc + 8 dec (same budget as Claude_15's 5 enc + 8 dec)
RefineNet / LearnedMerge / DCN unchanged from Claude_15.
Estimated: ~0.879M params, ~95G MACs
"""
import numpy as np
import torch
...
# ======================== SAFNet_Claude_16 ========================
class SAFNet_Claude_16(nn.Module):
    def __init__(self):
        super().__init__()
...
if __name__ == "__main__":
    ...
    model = SAFNet_Claude_16().to(device)
